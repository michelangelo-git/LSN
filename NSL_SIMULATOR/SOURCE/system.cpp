...
void System :: move(int i){ // Propose a MC move for particle i
    if(_sim_type == 3){ // Gibbs sampler for Ising
	    double sum_neighbors = _particle(this->pbc(i-1)).getspin() 
	    			   + _particle(this->pbc(i+1)).getspin(); // Calculate the sum of the neighboring spins
	    double P_up = 1.0 / (1.0 + exp( -2.0 *(_H + _J * sum_neighbors)/_temp)); // Calculate the probability for the spin to be +1
	    if(_rnd.Rannyu() < P_up) _particle(i).setspin(1); // Randomly determine the new spin state based on the calculated probability
	    else _particle(i).setspin(-1);
	    _naccepted++; // Gibbs sampling always accepts the move
  } else {           // M(RT)^2
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  _rnd.SetRandom(seed,p1,p2);
	...
	ifstream outdir("../INPUT/odir.dat");
	if(outdir.is_open()){
		outdir >> _odir;
		outdir >> _odir;
		cout << "Output directory: " << _odir << endl;
	}
	else{
		_odir = "../OUTPUT";
		cout << "Output directory set to default: " << _odir << endl;
		}
	outdir.close();
		
  ofstream couta(_odir + "/acceptance.dat"); // Set the heading line in file _odir/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input("../INPUT/input.dat"); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open(_odir + "/output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> _restart;
      if(!_restart) _input_config_path = "../INPUT/CONFIG/";
      else read_config_path();
...
void System :: initialize_velocities(){
  if(_restart and _sim_type==0){
    ifstream cinf;
    cinf.open(_input_config_path + "velocities.in");
    if(cinf.is_open()){
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> vx >> vy >> vz;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp(_odir + "/potential_energy.dat");
      ...
      }else if( property == "INSTANT_POTENTIAL_ENERGY" ){
        ofstream coutp(_odir + "/instant_potential_energy.dat");
        coutp.close();
        _measure_penergy_instant = true;
      } else if( property == "KINETIC_ENERGY" ){
        ofstream coutk(_odir + "/kinetic_energy.dat");
        ...
        ofstream coutt(_odir + "/total_energy.dat");
        ...
        ofstream coutte(_odir + "/temperature.dat");
        ...
        ofstream coutpr(_odir + "/pressure.dat");
        ...
        index_property++;
        _ptail = 3*32*M_PI*_rho*( 1/(9*pow(_r_cut,9)) -1/(6*pow(_r_cut,6)) );
      } else if( property == "GOFR" ){
        ofstream coutgr(_odir + "/gofr.dat");
        ...
        ofstream coutpr(_odir + "/magnetization.dat");
        ...
        ofstream coutpr(_odir + "/specific_heat.dat");
        ...
        ofstream coutpr(_odir + "/susceptibility.dat");
        ...
        ofstream coutf;
        coutf.open(_odir + "/output.dat",ios::app);
...
void System :: seek_temperature(){
	if(_sim_type == 0 && !_restart){
		double global_range = 0.03;
		double block_range = 0.1;
		double current_temp = 0;
		int block_size = 500;
		int consecutive_stable = 0;
		int global_stability = 20; // how many consecutive stable blocks to be contempt
		int search = 1;
		int MAX_search = pow(10,3);

		ofstream T(_odir+"seekT.dat");
		
		// first we let the system reach its current equilibrium:
		for(int stp = 1; stp <= block_size; stp++){
			for (int i=0; i<_npart; i++)
				current_temp += dot(_particle(i).getvelocity(),_particle(i).getvelocity()); 
			step();
		}
		current_temp /= (3.*double(_npart)*block_size);
		// roughly heat the system to get it in the right range
		change_vel(_temp - current_temp);
		//now refine the system to get to the right equilibrated temperature
			//global average check
			double globalAvg_temp = 0;
			for(int blk = 1; blk < global_stability; blk++){	
				// measure current temperature T = 2/3 total_kinetic_energy / num_particles
				// = v^2/(3*num_paricles)
				double block_temp = 0;
				for(int stp = 1; stp <= block_size; stp++){
					for (int i=0; i<_npart; i++)
						block_temp += dot(_particle(i).getvelocity(),_particle(i).getvelocity()); 
					step();
				}
				block_temp /= (3.*double(_npart)*block_size);
				globalAvg_temp += block_temp;
				// if(temperature under target) heat the system by adjusting velocities 
				//and restart the timer//
				//else if(temperature above target) cool the system by adjusting velocities
				//and reset timer		
				if(abs(block_temp - _temp) > block_range){
					int sign = 1;
					if(block_temp > _temp) sign = -1;
					if(abs(block_temp - _temp)> 1.2*block_range)
						change_vel(sign*block_range);
					else
						this->initialize_velocities();
					//reset timers and averages
					blk = 0;
					globalAvg_temp = 0;
				}
				else if(abs(globalAvg_temp/blk - _temp) > 1.5*global_range)
					this->initialize_velocities();

				// block average check after some blocks
				if(blk >= 15){
					if(abs(globalAvg_temp / double(blk) - _temp) > global_range){
						cout << globalAvg_temp << " blk: " << blk 
						<< " Avg: "<< globalAvg_temp / double(blk)<< endl;
						blk = 0;
						globalAvg_temp = 0;
						
					}
				}
				T << block_temp << " " << globalAvg_temp/blk << endl;
			//cout << "\nEquilibrated starting temperature = " << current_temp << endl;
			}
			T.close();
		}
		else{
			for(int t = 0; t < 5000; t++) step();
		}
}

void System :: change_vel(double hot_cold){
  for (int i=0; i<_npart; i++){
    double new_vx = _particle(i).getvelocity(0) * (1. + hot_cold);
    double new_vy = _particle(i).getvelocity(1) * (1. + hot_cold);
    double new_vz = _particle(i).getvelocity(2) * (1. + hot_cold);
    _particle(i).setvelocity(0, new_vx);
    _particle(i).setvelocity(1, new_vy);
    _particle(i).setvelocity(2, new_vz );
  }
   double xold, yold, zold;
    for (int i=0; i<_npart; i++){
      xold = this->pbc( _particle(i).getposition(0,true) - _particle(i).getvelocity(0)*_delta, 0);
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(_odir + "/CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.open(_odir + "/CONFIG/config.spin");
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open(_odir + "/CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: write_velocities(){
  ofstream coutf;
  coutf.open(_odir + "/CONFIG/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
...
void System::read_config_path(){
	ifstream _path;
	_path.open("../INPUT/idir.dat");
	if(_path.is_open()){
		_path >> _input_config_path >> _input_config_path;
		cout << "CONFIG path: " << _input_config_path << endl;			
	}
	else	cerr <<"PROBLEM: Unable to read CONFIG path" << endl;
	_path.close();
	return;
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open(_input_config_path + "config.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
  ...
  if(_restart and _sim_type > 1){
    int spin;
    cinf.open(_input_config_path + "config.spin");
    for(int i=0; i<_npart; i++){
      cinf >> spin;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    ...
    for (int i = 0; i < _npart-1; i++) {
      for (int j = i+1; j < _npart; j++) {   
        ...
        distance(1) = this->pbc(_particle(i).getposition(1,true) - _particle(j).getposition(1,true), 1);
        distance(2) = this->pbc(_particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt(dot(distance, distance));  // Euclidean distance
        
        // GOFR: Update g(r) histogram
        if(_measure_gofr) {
        	int bin_index = int(dr / _bin_size);  // Determine the appropriate bin
         if (bin_index < _n_bins)
            _measurement(_index_gofr + bin_index) += 2;  // Increment by 2 to account for symmetry
        }
        
        // Measure potential energy and pressure if within cutoff radius
        if (dr < _r_cut) {
          if (_measure_penergy) 
            penergy_temp += 1.0 / pow(dr, 12) - 1.0 / pow(dr, 6);  // Lennard-Jones potential
          if (_measure_pressure) 
            pressure_temp += 1.0 / pow(dr, 12) - 0.5 / pow(dr, 6);  // Pressure calculation
        }
      }
    }
   }
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    penergy_temp = _measure_tails*_vtail + 4.0 * penergy_temp / double(_npart);
    _measurement(_index_penergy) = penergy_temp;
    // EXERCISE 7
    if(_measure_penergy_instant){ //to improve: resize as steps and empty it at measure  
		 ofstream coutf;
		 coutf.open(_odir + "instant_potential_energy.dat",ios::app);
		 coutf << penergy_temp << endl;
		 coutf.close();
    }
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if(_measure_pressure) _measurement(_index_pressure) = _measure_tails*_ptail + _rho*_temp + 48*pressure_temp/(_volume*3);
  // MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
   if(_measure_magnet){
   	_measurement(_index_magnet) = 0; //new measure
   	for(int i=0; i < _npart; i++)
   		_measurement(_index_magnet) += _particle(i).getspin();
   	_measurement(_index_magnet) /= _npart;
   }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if(_measure_cv){
  	_measurement(_index_cv) = 0;
  	double s_i, s_j;
  	for(int i=0; i < _npart; i++){
  		s_i = double(_particle(i).getspin());
  		s_j = double(_particle(this->pbc(i+1)).getspin());
  		_measurement(_index_cv) += -_J * s_i * s_j - 0.5 * _H * (s_i + s_j);
  	}
  	_measurement(_index_cv) = pow(_measurement(_index_cv), 2); // = H^2, calcolo il vero Ct in averages. manca /T^2
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi){
  	_measurement(_index_chi) = 0;
   	for(int i=0; i < _npart; i++)
   		_measurement(_index_chi) += _particle(i).getspin(); //iff h=0
   	_measurement(_index_chi) = pow(_measurement(_index_chi), 2)/_temp;
   }
// TO BE FIXED IN EXERCISE 6
...
void System :: averages(int blk){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open(_odir + "/potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(_odir + "/kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open(_odir + "/total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(_odir + "/temperature.dat",ios::app);
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
   if (_measure_pressure){
    coutf.open(_odir + "/pressure.dat",ios::app);
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
	if (_measure_gofr){
		if(blk == _nblocks){
		string _sim_type_name = "";
		if(_sim_type == 0)	_sim_type_name = "VERLET";
		else if(_sim_type == 1)	_sim_type_name = "METRO";
						 
		string filename = _odir + "gofr_" + _sim_type_name + ".dat";
		coutf.open(filename);
    for (int i = 0; i < _n_bins; i++) {
        double shell_volume = 4.0 * M_PI * pow((i+1)*_bin_size, 2) * _bin_size;  // Volume of the spherical shell
		  double normalization = _npart *_rho * shell_volume;
		 sum_average = _global_av(_index_gofr+i) / normalization;
		 sum_ave2 = _global_av2(_index_gofr+i) / pow(normalization,2);
		 coutf << setw(12) << (i+1)*_bin_size
		       << setw(12) << sum_average/double(blk)
		       << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      }
      coutf.close();
  ...
  // TO BE FIXED IN EXERCISE 6
  if (_measure_magnet){
    coutf.open(_odir + "/magnetization.dat",ios::app);
  ...
  // TO BE FIXED IN EXERCISE 6
  if (_measure_cv){
    coutf.open(_odir + "/specific_heat.dat",ios::app);
    average = (_average(_index_cv) - pow(_npart*_average(_index_tenergy),2)) / (_temp*_temp);
    _global_av(_index_cv) += average - _average(_index_cv);
    _global_av2(_index_cv) += pow(average,2) - pow(_average(_index_cv), 2);
  ...
  // TO BE FIXED IN EXERCISE 6
  if (_measure_chi){
    coutf.open(_odir + "/susceptibility.dat",ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open(_odir + "/acceptance.dat",ios::app);
